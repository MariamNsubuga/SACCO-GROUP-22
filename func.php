...
<?php
function textarea(){
	$lines = file('test2.txt',FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
...
echo "</table>";
	...
	list($a,$b,$c,$d)=explode(" ",$file,4);
		...
		if($a=="contribution"){
			$Status="Approved";
		$query=mysqli_query($con,"insert into contributions(name,Amount,ReceiptNumber,Status) values('$b','$c','$d','$Status')");
		if($query){
		...
		$mber=mysqli_query($con,"select count(name) from contributions where name='$b'");
			$row = mysqli_fetch_array( $mber);
			if($row<2)
			{
			echo "'$b' is not a regular contributor";
			}else
			{
			$sumcon=mysqli_query($con,"select SUM(Amount) AS Amount FROM contributions where name='$b'");
				$row = mysqli_fetch_array( $sumcon);
				$lcon = $row['Amount'];
			$half=0.5*$lcon;
				...
				$status=mysqli_query($con,"update LoanRequest set Status='Appoved' where name='$b' ");
				}else if($c>$half)
				{
						$query=mysqli_query($con,"insert into LoanRequest(Name,Amount,Date) values('$b','$c','$d')");
				if($query){
				...
				$status=mysqli_query($con,"update LoanRequest set Status='Denied' where name='$b' ");
				}
			} 
		}
		else if($a=="idea"){
			$Status="Pending";
		$query=mysqli_query($con,"insert into Idea(Name,Description,InitialCapital,Status) values('$b','$c','$d','$Status')");
		if($query){
...
function addcontributions(){
	...
	$con=mysqli_connect("localhost","root","","Sacco");
	$Status='Approved';
	$r="select SUM(Amount) FROM contributions where Status='$Status'";
...
<?php
	...
	//new
	$w="update Idea  set Status='$Status' where Description='$Description'";
	//old
	$query = mysqli_query($c,$w);
	if(!$query){
		echo'idea doesnot exist';
	}
}		
?>
<?php 
...
<table>
	...
	<tr>
		...
		<div><textarea><?php addloans(); ?></textarea></div>
<?php
}
?>
...
<?php
function addloans(){
//adding total amount in loans
$c=mysqli_connect("localhost","root","","Sacco");
$r="select SUM(Amount) FROM loanrequest";
...
<h2>Idea Updates</h2>
	<table id="table2">
		<tr>
			<td id = "des">Description</td> <td><input type = "text" name ="Description" id="can"/></td></tr>
			<tr><td id = "des">Status</td> <td><input type = "text" name ="Status" id="can" /></td>
		</tr>
 			<tr>
 				<td><input type="submit" name="update" value = "UPDATE" id="submit"/></td>
 				<td><input type="submit" name="delete" value = "DELETE" id="submit"/></td>
 			</tr>
	</table><br/><br/>
